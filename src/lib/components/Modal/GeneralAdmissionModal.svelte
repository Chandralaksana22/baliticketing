<script lang="ts">
	...
	const filteredProducts = explorerGA.filter(
		(product: { type: string }) => product?.type === 'General Admission'
	);
	const filteredProductsbalifarmGA = balifarmGA.filter(
		(product: { type: string }) => product?.type === 'General Admission'
	);
	const filteredProductsbalisafariGA = balisafariGA.filter(
		(product: { type: string }) => product?.type === 'General Admission'
	);
	const filteredProductsbalibirdGA = balibirdGA.filter(
		(product: { type: string }) => product?.type === 'General Admission'
	);
</script>
		...
		<div class="p-4 md:p-5 space-y-4">
			...
			<div id="default-tab-content">
				...
				>
					...
					>
						<!-- Svelte loop for products -->
						{#if filteredProductsbalifarmGA.length > 0}
							{#each filteredProductsbalifarmGA as product}
								<div>
									<a
										data-sveltekit-reload="off"
										href={`/vendor/${product?.vendor.slug}/${product?.slug}`}
				...
				>
					...
					>
						<!-- Svelte loop for products -->
						{#if filteredProductsbalisafariGA.length > 0}
							{#each filteredProductsbalisafariGA as product}
								<div>
									<a
										data-sveltekit-reload="off"
										href={`/vendor/${product?.vendor.slug}/${product?.slug}`}
				...
				>
					...
					>
						<!-- Svelte loop for products -->
						{#if filteredProductsbalibirdGA.length > 0}
							{#each filteredProductsbalibirdGA as product}
								<div>
									<a
										data-sveltekit-reload="off"
										href={`/vendor/${product?.vendor.slug}/${product?.slug}`}
