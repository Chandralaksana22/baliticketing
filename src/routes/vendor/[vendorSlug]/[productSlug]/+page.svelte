<script lang="ts">
	...
	function toggleAccordion(index) {
		if (openAccordionIndex === index) {
			openAccordionIndex = -1; // Close if already open
		} else {
			openAccordionIndex = index; // Open accordion with the selected index
		}
	}
		...
		>
			<div class="col-span-2">
				<img
					...
					class="rounded-lg h-[25vh] lg:h-[52vh] w-full object-cover"
				/>
			</div>
							...
							<div class="grid grid-cols-3 gap-4">
								<div class="dropdown">
									<div
										tabindex="0"
										role="button"
										class="text-white bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-3 p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 flex items-center justify-between"
									>
									...
									>
										<li class="grid grid-cols-3 lg:grid-cols-2">
											...
											<div class="block lg:hidden py-4">
												<div class="font-semibold text-blue text-sm">IDR 355,500</div>
											</div>
											<div class="pe-3 py-4">
												<div class="flex items-center">
													<button
										...
										<li class="grid grid-cols-3 lg:grid-cols-2">
											...
											<div class="pe-3 py-4">
												<div class="flex items-center">
													<button
														class="inline-flex no-color-change items-center justify-center p-1 me-3 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
														type="button"
													...
													<button
														class="inline-flex no-color-change items-center justify-center h-6 w-6 p-1 ms-3 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
														type="button"
								...
								<button
									type="submit"
									class="text-white w-full p-2.5 text-sm rounded-lg bg-blue font-bold text-center"
							...
							<div class="my-5">
								...
								<div class="flex gap-4 mt-1">
									...
									<p class="text-xs text-black">
										{adultCount} adult, {childrenCount} children, and {infantCount} infants
									</p>
								</div>
								<div class="flex gap-4 mt-1">
									<svg
								...
								<form on:submit={handleSubmit}>
									<input type="hidden" name="id" bind:value={selectedTicket.id} />
									<input type="hidden" name="name" bind:value={selectedTicket.name} />
									<input type="hidden" name="slug" bind:value={selectedTicket.slug} />
									<input type="hidden" name="adults" bind:value={adultCount} />
									<input type="hidden" name="children" bind:value={childrenCount} />
									<input type="hidden" name="infants" bind:value={infantCount} />
									<input type="hidden" name="date" bind:value={date} />
									<input type="hidden" name="adult_price" bind:value={selectedTicket.adult_price} />
									<input
										type="hidden"
										name="infant_price"
										bind:value={selectedTicket.infant_price}
									/>
									<input
										type="hidden"
										name="chlidren_price"
										bind:value={selectedTicket.children_price}
									/>
									<input
										type="hidden"
										name="total_price"
										value={(adultCount * selectedTicket?.adult_price +
											childrenCount * selectedTicket?.children_price +
											infantCount * selectedTicket?.infant_price) *
									...
									<button
										type="submit"
										class="text-white w-full mt-5 p-2.5 text-sm rounded-lg bg-blue font-bold text-center"
										>Book Ticket</button
									>
								</form>
							</div>
